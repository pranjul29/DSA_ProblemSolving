...
public class Seats {
    public static int seats(String A) {
        ...
        int middle = occupied.size()/2;
        int jumps = 0;
        for(int i = 0;i<middle;i++)
        {
            jumps += ((occupied.get(middle)-occupied.get(i) - 1) - (middle-i-1));
            jumps = jumps%mod;
        }
        for(int i = middle+1;i<occupied.size();i++)
        {
            jumps += ((occupied.get(i)-occupied.get(middle) - 1) - (i-middle-1));
            jumps = jumps%mod;
        }
        return jumps;
    }
    public static void main(String[] args) {
        String A = new String("xx.....xx.x..xxxx..xxxx.xx..xx..x.xxxx");
        // Expected: 79
