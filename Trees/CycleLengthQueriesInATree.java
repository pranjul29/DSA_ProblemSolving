/* 
Source: https://leetcode.com/problems/cycle-length-queries-in-a-tree/description/
...
public class CycleLengthQueriesInATree {
    static int findCommonRoot(int n,int m)
    {
        if(n == 1 || m == 1)
            return 1;
        while(true)
    ...
    public static int[] cycleLengthQueries(int n, int[][] queries) {
        ...
        for(int i = 0;i<queries.length;i++)
        {
            int a = queries[i][0];
            int b = queries[i][1];
            int root = findCommonRoot(a,b);
            int h1 = findHeight(a);
            int h2 = findHeight(b);
            int h = findHeight(root);
            result[i] = (h2-h) + (h1-h) + 1;
        }
        return result;
    }
    public static void main(String[] args) {
        ...
        int[] result = cycleLengthQueries(n,queries);
    }
}
